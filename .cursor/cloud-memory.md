...
- **What exists now (frontend):**
  ...
  - **`components/pyramid/scroll-video.tsx`** — **Glass Hero Plate** (master-plan override: rounded-3xl applied). Single element serves as both watermark obliterator and typography container. Position: `absolute left:50% translateX(-50%) bottom:10%`. Size: `width:85% maxWidth:1024px minHeight:280px padding:48px`. Glass: `borderRadius:24px blur(40px) rgba(255,255,255,0.10) border rgba(255,255,255,0.20)`. Layout: `flex-col items-center text-center zIndex:20`. Contains: HUD status row (lime dot + `[ PIRAMIDA BACKSTAGE // FEED ACTIVE ]` at opacity 0.38) → overlayLabel → H1 `clamp(48px,6vw,88px)` → subtitle. Frame counter + children slot: `zIndex:30`. Corner mask (Ezgif): `absolute bottom:4px right:0 224×40px concrete-black zIndex:4` — untouched.
