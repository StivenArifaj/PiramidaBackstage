...
- **What exists now (frontend):**
  - **`app/(marketing)/page.tsx`** (served at `/`; `app/page.tsx` deleted, restoring master-plan §5) — DUAL flythrough landing: (1) `/frames/hero/` 300 frames reversed → interlude 3-col block → (2) `/frames/detail-sample/` 265 frames reversed → manifesto + features + footer CTA. Root div `overflowX: 'clip'`.
  ...
  - **`components/ui/image-gallery.tsx`** (NEW) — brutalist gallery: SVG 30°-hatch placeholder, crosshair overlay, view-label + counter, ← → nav, 6-up thumb strip with lime accent bar.
  - **`components/pyramid/scroll-video.tsx`** — adds Option-A watermark mask: solid 224×40px `concrete-black` block at `bottom:4px right:0`, zIndex 4, "[MASK // WM]" JetBrains Mono label. Covers bottom-right Ezgif attribution in all 565 frames.
...
- **Watermark masking — Option A (hard block)**: 224×40px solid `var(--color-concrete-black)` div, `position: absolute, bottom: 4px, right: 0`, zIndex 4. Covers the Ezgif.com attribution watermark in bottom-right corner of all 565 frames (300 hero + 265 detail-sample). The "BSM · BLEC!STRAKOSHA MEDIA 4K" centre watermark is the original videographer's mark and cannot be masked without obscuring main video content — it is not covered.
