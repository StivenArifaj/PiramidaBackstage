...
- **Inventory CRUD:** GET/POST /api/assets (Supabase + mock fallback). PATCH/DELETE /api/assets/[id]. Inventory page has + Add Asset modal and Edit/Delete row actions.
- **Maintenance:** GET/POST /api/maintenance + PATCH /api/maintenance/[id]. maintenance_logs table in Supabase migration 0004. Maintenance page groups spaces by floor with status badges and Schedule Maintenance modal.
- **BI Reports:** GET /api/reports?date_range=&space_codes=. Reports page has dark filter bar (multi-select spaces, date range), KPI summary, booking log table.
