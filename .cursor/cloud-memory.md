...
- **What exists now (frontend):**
  ...
  - **`components/pyramid/scroll-video.tsx`** — two watermark masks: (a) hard black block: 224×40px `concrete-black` at `bottom:4px right:0` zIndex 4 covers Ezgif corner attribution; (b) large glassmorphic block: `bottom:15% width:80% maxWidth:900px height:10rem`, `blur(24px) rgba(0,0,0,0.20) border rgba(255,255,255,0.10)`, label + lime dot CENTERED inside — covers the full "BSM 4K BLEDI STRAKOSHA MEDIA" lower-centre watermark, zIndex 3.
