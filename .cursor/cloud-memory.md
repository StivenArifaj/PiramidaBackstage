...
- **What exists now (frontend):**
  - **`app/(marketing)/page.tsx`** (served at `/`; `app/page.tsx` deleted, restoring master-plan §5) — DUAL flythrough landing: BrandStrip → Flythrough 1 (`/frames/hero/` 300 frames reversed, stats panel + stacked CTAs at bottom:64px) → Interlude 3-col block → Flythrough 2 (`/frames/detail-sample/` 265 frames reversed, "open floor plans →" at bottom:64px). All marketing copy purged. Root div `overflowX: 'clip'`.
  ...
  - **`components/pyramid/scroll-video.tsx`** — two watermark masks: (a) Option-A hard block: 224×40px `concrete-black` at `bottom:4px right:0` zIndex 4 covers Ezgif corner attribution in all 565 frames; (b) centre blur: 420×88px `backdropFilter:blur(12px)` at `left:50% top:58%` zIndex 3 covers "BLEC!STRAKOSHA MEDIA 4K" videographer watermark (authorised design-system exception). CTA overlay children use `bottom:64px` to clear hard mask.
...
- [2026-06-20] Claude Code (claude-sonnet-4-6) [Aron/frontend] — UI Bug Fixes, Content Purge & Centre Watermark: (1) PURGED all marketing copy from `app/(marketing)/page.tsx` — deleted manifesto ("30 emails. 4 spreadsheets..."), building-data table, feature strip (4-col grid), footer CTA ("Replace 30 emails"), both SectionDividers, FEATURES + BUILDING_SPECS constants; landing page is now purely BrandStrip → Flythrough 1 → Interlude → Flythrough 2; (2) FIXED button overlap — both CTA containers in landing ScrollVideo blocks raised from `bottom: 18px` to `bottom: 64px`, clearing the `[MASK // WM]` block top edge at 44px (4px offset + 40px height); (3) ADDED centre watermark blur to `components/pyramid/scroll-video.tsx` — 420×88px `backdropFilter: blur(12px)` div at `left:50% top:58%` (derived from cover-fit screen position of "BLEC!STRAKOSHA MEDIA 4K" text in all 565 frames), `backgroundColor: rgba(255,255,255,0.07)`, zIndex 3, `pointerEvents:none`; authorised exception to design system. `npx tsc --noEmit` clean.
