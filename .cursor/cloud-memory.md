...
- **What exists now (frontend):**
  ...
  - **`components/pyramid/scroll-video.tsx`** — two watermark masks: (a) hard black block: 224×40px `concrete-black` at `bottom:4px right:0` zIndex 4 covers Ezgif corner attribution; (b) **liquid glass block**: `bottom:8% width:90% maxWidth:800px height:180px borderRadius:24px blur(40px) rgba(255,255,255,0.05) border rgba(255,255,255,0.10) shadow-2xl` at **zIndex:10** — covers BSM text; sits BEHIND the text block which is now at **zIndex:20**. Children slot wrapped in `position:absolute inset:0 zIndex:20`. H1 title + overlays always render on top of the glass. Label dimmed to opacity 0.38.
...
- **Last updated:** 2026-06-20, Claude Code (claude-sonnet-4-6) [Aron/frontend] — Liquid glass watermark + z-index fix.
